const EventEmitter = require("events"),
  ...
  setProfileName = require("kik-node-api/src/requests/setProfileName"),
...
module.exports = class KikClient extends EventEmitter {
    ...
    this.connection = new KikConnection(this.logger, err => {
      if (err) {
        ...
        this.session = this.session
          ? this.session
          : sessionUtils.getSession(this.params.username);
        ...
        this.vidManager = new VideoManager(
          this.params.username,
          this.params.password,
          this.session.node,
          true
  ...
  sendMessage(jid, msg, callback) {
    this.logger.log(
      "info",
      `Sending ${
        jid.endsWith("groups.kik.com") ? "group" : "private"
      } message to ${jid} Content: ${msg}`
    );
  ...
  async sendVideo(
    ...
    if (!Buffer.isBuffer(vidPath)) {
      loop = callback;
      autoplay = loop;
      allowSaving = autoplay;
      allowForwarding = allowSaving;
      imgPath = allowForwarding;
    }
    this.logger.log(
      "info",
      `Sending ${
    ...
    const video = await this.vidManager.uploadVid(
      vidPath,
      this.params.version,
      this.logger,
      imgPath
  ...
  async sendSticker(jid, imgPath, allowForwarding, allowSaving, callback) {
    ...
    const image = await this.stickerManager.uploadImg(
      imgPath,
      this.params.version
    );
    let req = sendSticker(
  ...
  setAdmin(groupJid, userJid, bool) {
    this.logger.log(
      "info",
      `Setting admin = ${bool} for jid ${userJid} in group ${groupJid}`
    );
  ...
  setBanned(groupJid, userJid, bool) {
    this.logger.log(
      "info",
      `Setting banned = ${bool} for jid ${userJid} in group ${groupJid}`
    );
  ...
  setGroupMember(groupJid, userJid, bool) {
    this.logger.log(
      "info",
      `Setting member = ${bool} for jid ${userJid} in group ${groupJid}`
    );
  ...
  setGroupName(groupJid, groupName) {
    this.logger.log(
      "info",
      `Setting group name to ${groupName} for group ${groupJid}`
    );
